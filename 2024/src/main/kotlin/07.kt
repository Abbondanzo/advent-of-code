...
private fun isValidEquationPartTwo(total: Long, items: List<Long>): Boolean {
  ...
  if (items.size == 2) {
    return total == items[0] + items[1] ||
        ...
        total == "${items[0]}${items[1]}".toLong()
  }
  return isValidEquationPartTwo(total - items.last(), items.dropLast(1)) ||
      (total % items.last() == 0L &&
          isValidEquationPartTwo(total / items.last(), items.dropLast(1))) ||
      (total.toString().endsWith(items.last().toString()) &&
          isValidEquationPartTwo(
              total.toString().removeSuffix(items.last().toString()).toLong(), items.dropLast(1)))
}
